...
node2wayids(NodeID) ->
  case eredis:q(routing, ["HGET", routing, node2way_key(NodeID)]) of
    {ok,undefined} -> [];
    {ok, Ways} -> binary_to_term(Ways)
  end.
  
lookup_way(WayID) ->
  case eredis:q(routing, ["HGET", routing, way_key(WayID)]) of
    {ok,undefined} -> undefined;
    {ok, Way} -> binary_to_term(Way)
  end.
  
lookup_node(NodeID) ->
  case eredis:q(routing, ["HGET", routing, node_key(NodeID)]) of
    {ok,undefined} -> undefined;
...
store_way(Way=#way{id=ID, refs=Refs}) ->
  ...
  eredis:q(routing, ["HSET", routing, way_key(ID), term_to_binary(Way)]).
  
store_node(Node=#node{id=ID}) ->
  eredis:q(routing, ["HSET", routing, node_key(ID), term_to_binary(Node)]).
...
store_nodeid2wayid(NodeID, WayID) ->
  eredis:q(routing, ["HSET", routing, node2way_key(NodeID), term_to_binary([WayID|node2wayids(NodeID)])]).
